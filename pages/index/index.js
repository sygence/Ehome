...
Page({
  ...
  onLoad: function () {
    ...
    wx.checkSession({
      ...
      console.log('储存的token', wx.getStorageSync("token"))
    console.log('储存的用户名',wx.getStorageSync('nickName'))
    console.log('储存的openid',wx.getStorageSync('openid'))
    //请求登录
    wx.request({
      ...
      success: function (resp) {
        ...
        if (resp_dict.err_code == 0) {
          ...
          wx.setStorage({
            key: 'username',
            data: wx.getStorageSync('nickName'),
          })
        } else {
          getApp().showSvrErrModal(resp);
        }
        ...
        console.log('resp打印',resp_dict.err_code)
  ...
  navigateToPost() {
    wx.navigateTo({
      url: '/pages/add_forum_article/add_forum_article',
    })
